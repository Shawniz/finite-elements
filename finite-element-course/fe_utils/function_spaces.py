...
from .finite_elements import LagrangeElement, FiniteElement, lagrange_points, VectorFiniteElement
...
class FunctionSpace(object):
    ...
    def __init__(self, mesh, element):
        ...
        self.cell_nodes = np.zeros(shape=(mesh.entity_counts[-1], element.node_count), dtype=int)

        # Calculate starting global indices for nodes in each type of entity
        for cell in range(mesh.entity_counts[-1]):
            #Make sure to include dimension as well
            for delta in range(dim + 1):
                for eps in range(len(element.entity_nodes[delta])):
                    i = mesh.adjacency(dim, delta)[cell, eps]
                    G = i * element.nodes_per_entity[delta] + np.dot(element.nodes_per_entity[:delta], mesh.entity_counts[:delta])
                    self.cell_nodes[cell, element.entity_nodes[delta][eps]] = [k for k in range(G, G + element.nodes_per_entity[delta])]
...
class Function(object):
    ...
    def interpolate(self, fn):
        ...
        for c in range(fs.mesh.entity_counts[-1]):
            ...
            #VFE and FE case
            self.values[fs.cell_nodes[c]] = [np.dot(fn(node), fs.element.node_weights[i % fs.element.dimension]) for i, node in enumerate(node_coords)] if isinstance(fs.element, VectorFiniteElement) else [fn(node) for node in node_coords] 
    ...
    def integrate(self):
        ...
        for c in range(mesh.entity_counts[-1]):
            ...
            #F evaluated
            F = self.values[fs.cell_nodes[c, :]]

            # Compute the actual cell integral.
            comp = np.einsum('i, qi, q', F, phi, Q.weights)
            integral += comp * detJ
