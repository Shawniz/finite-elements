...
from .finite_elements import LagrangeElement, FiniteElement
...
class Mesh(object):
    ...
    def __init__(self, vertex_coords, cell_vertices):
        ...
        #gradient of basis functions never changes
        fe = LagrangeElement(self.cell, 1)
        self.grad = fe.tabulate(points=np.array([[0.] * self.dim]), grad=True)
    ...
    def adjacency(self, dim1, dim2):
        ...
        """
        #Adjustment for dim1 == dim2 case
        if dim1 == dim2:
            n_entities = self.entity_counts[dim1]
            return np.arange(n_entities).reshape(-1, 1)
    ...
    def jacobian(self, c):
        ...
        """
        return np.einsum("ij, ji... -> j...", self.vertex_coords[self.cell_vertices[c], :], self.grad)
